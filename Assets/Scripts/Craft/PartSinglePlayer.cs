...
public class PartSinglePlayer : MonoBehaviour
{
	[Header("Part")]
	[SerializeField]
	...
	[SerializeField]
	protected BaseSinglePlayer mainBaseSinglePlayer;
	...
	public virtual void Start()
	{
		...
		foreach (Builder b in FindObjectsByType<Builder>(FindObjectsSortMode.None))
		{
			...
			if (mainBase)
				...
				mainBaseSinglePlayer = b.transform.parent.GetComponent<BaseSinglePlayer>();
			//b.InitializePart(gameObject);
			if (GetComponent<ControlModule>() || GetComponent<ControlModuleSinglePlayer>())
			{
				...
				b.currentHitCollider = GetComponentInChildren<Collider>();
			}
			else
			{
				try
				...
				{
					transform.parent = b.craftHead.transform;
				}
				...
				if (GetComponent<Part>())
					GetComponent<Part>().attachedCollider = b.currentHitCollider.GetComponent<Collider>();
				else
					GetComponent<PartSinglePlayer>().attachedCollider = b.currentHitCollider.GetComponent<Collider>();
			}
			break;
		}
	}
	...
	void DestroySelf()
	{
        ...
        if (mainBase)
			...
			mainBaseSinglePlayer.DestroyPart(gameObject, armed, transform.position, explosionRadius, explosionDamage, GetComponent<ControlModuleSinglePlayer>() ? 1 : 0);
	}
